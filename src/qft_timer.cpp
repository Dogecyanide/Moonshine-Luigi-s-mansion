...
#undef STATUS
  ...
  bool missionCounterOnScreen(const TGCConsole2 *console) {
    const J2DPane *balloons = hudPane(console, '\0b_0');
    const J2DPane *reds = hudPane(console, '\0r_0');
    // The counter roots are tiny anchors; their children enter first.
    return (balloons && balloons->mIsVisible) ||
           (reds && reds->mIsVisible);
  }
