...
- `src/*.cpp` — mod source, auto-globbed by CMake:
  ...
  - `settings.cpp` / `settings.hxx` — persistent mod settings, independent of the menu that edits them and the features that read them. A setting is one row in `include/susamune/settings_list.h`'s `SUSAMUNE_SETTING_LIST` X-macro (enumerator + stable ini key) plus one row in `kSettingDescs` (name, type BOOL/CHOICE, default, `SettingCategory`) plus one byte of live value in the global `gSettings`; the menu renders/edits them generically, `savestate.cpp` and `features.cpp` read them. Settings are grouped by `SettingCategory` (Savestate / QoL / Cosmetic / Misc), one menu tab each. Persisted to `susamune.ini` on the SD card — see "Settings persistence" below.
...
**Adding a new setting:** append a row to `SUSAMUNE_SETTING_LIST` in `include/susamune/settings_list.h` (enumerator + stable ini key) and a matching row in `kSettingDescs` (`settings.cpp`) — name, `SETTING_BOOL`/`SETTING_CHOICE`, choice count + labels, default. **Append only** — the list order is the on-disc layout (see "Settings persistence"). Nothing else changes: the enum, the menu tab, the ini key table on both sides, and `SettingsData::values` are all generated off the list, and a `static_assert` catches a list row added without a descriptor row. Features read it via `gSettings.get*`.

**Adding a new bind:** append a row to `SUSAMUNE_BIND_LIST` in `include/susamune/binds_list.h` (enumerator + stable ini key) and a matching row in `kBindDescs` (`binds.cpp`) — menu label + default combo. **Append only**, for the same reason settings are. Nothing else changes: the enum, the Binds tab, the ini key table on both sides and the MEM2 `binds[]` layout all follow from the list. Read it from an action with `gBinds.wasPressed(id)` / `gBinds.isHeld(id)`.
