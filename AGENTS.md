...
- `src/*.cpp` — mod source, auto-globbed by CMake:
  ...
  - `settings.cpp` / `settings.hxx` — persistent mod settings, independent of the menu that edits them and the features that read them. Each setting is one row in `kSettingDescs` (name, type BOOL/CHOICE, default, `SettingCategory`) plus one byte of live value in the global `gSettings`; the menu renders/edits them generically, `savestate.cpp` and `features.cpp` read them. `SettingsData` is a POD blob (magic+version) sized for future memory-card / SD serialization. Settings are grouped by `SettingCategory` (Savestate / QoL / Cosmetic / Misc), one menu tab each.
