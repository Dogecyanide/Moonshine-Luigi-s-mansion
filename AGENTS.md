...
- `include/susamune/binds_list.h` — the same arrangement for binds: `SUSAMUNE_BIND_LIST(X)` (one `(enumerator, ini_key)` row per bindable action, **append-only** for the same reason) plus `SUSAMUNE_BIND_BUTTON_LIST(X)`, the `(button bit, token)` table both sides use to spell a combo. Binds live in a `[binds]` section as `+`-joined tokens (`regrab_object = X+DUp`) rather than as a number, so the file stays hand-editable.
...
`cmake -B build -G Ninja` works too; the presets just bake in Ninja, the build dir and the option set (`savestates_console`/`savestates_emu`/`dev_console`/`dev_emu`). The Visual Studio generator is rejected.
