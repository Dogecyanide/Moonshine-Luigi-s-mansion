...
- `src/*.cpp` — mod source, auto-globbed by CMake:
  - `main.cpp` — hook entry points. `onAppInit` replaces the `initialize()` call in `main()`, i.e. the last point before the app state machine (logo → title → gameplay) starts, and is where settings are made live; the first `onUpdate` constructs the persistent menu/savestate objects after the system font and final stage heap exist, then runs each frame in place of `mDirector->direct()`; `afterDraw` runs at the end of rendering and processes queued savestate loads after the game's `GXDrawDone` barrier, `onSetup` runs once per stage load, and `onUpdateGameMode` runs in the game-mode loop. Also holds `getArenaLo` (heap-reservation hook, see below). Two ported gecko codes are implemented here as game logic rather than memory patches, because they act on the app/director state machines: **Intro Skip** (`onAppInit` pre-sets the boot `sGameInit` bit so the logo director never runs; `onUpdate` reports "title screen" during the intro-movie app state) and **Stage Intro Skip** (`onUpdate` raises `TMarDirector::mTickBudget` while `mCurState == STATE_INTRO_PLAYING`, buying extra logic ticks per rendered frame = fast-forward).
...
- `SUSAMUNE_MOD_REGION_SIZE` in `include/susamune/addresses.hxx` **must equal** `mod_region_size` in `patches.py` (currently `0x8000`), and `SUSAMUNE_SCRATCH` **must equal** `mod_scratch_size` (`0x40`).
...
1. **Boot** — `SusamuneCfgInit()` runs from the kernel's `main()` right after `ConfigInit()`, before the game is patched or started. It parses `/susamune.ini` into the block and stamps magic/version/count. Unset entries are `SUSAMUNE_CFG_UNSET` (`0xFF`).
...
3. **Save** — closing the menu with `gSettings.dirty()` stages values into the block, bumps `saveSeq`, and returns immediately. The kernel's main loop sees `saveSeq != ackSeq`, rewrites the whole ini, and answers with `status` (a FatFS `FRESULT`) + `ackSeq`. The mod polls that from `Menu::update` and toasts the result.
...
- **First boot writes the file from the mod, not the kernel.** Defaults live in `kSettingDescs`; duplicating them in the launcher would be a second source of truth. So when no ini exists the kernel raises `SUSAMUNE_CFG_FLAG_NO_FILE` and `Settings::init()` answers with a `save()`, creating a complete default file.
- **A save rewrites the entire file**, so unrecognised hand-added keys are dropped. Nothing is lost today because the reserved `[nintendont]` section (for the ISO path and the other loader flags, eventually) has no keys yet. Preserving unknown lines is the v2 refinement if that changes.
...
- **Reserved savestate MEM2 window** — `include/susamune/mem2_map.h` packs the 3 MB ISO/RealDI cache and all relocated Nintendont scratch/handoff buffers below physical `0x11F00000`, reserves physical `0x11F00000–0x12EF0000` exclusively for the snapshot and `0x12EF0000–0x12F00000` for the settings block, and places the ARM kernel at `0x12F00000`. The loader uses cached `0x9xxxxxxx` aliases; the kernel uses physical `0x1xxxxxxx` addresses. `launcher/mem_map.txt` documents the same layout.
