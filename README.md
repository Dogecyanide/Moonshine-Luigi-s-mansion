...
`Full-State Experimental 0.3.10` is the current hardware-testable state build.
...
- The slot captures the complete secondary gameplay heap, its allocator and
  ...
  main-loop control pair, the verified room/map flag slice, and libc RNG
  state.
...
```text
build-lm-diag/Moonshine-Luigis-Mansion-Full-State-Experimental-0.3.10.zip
...
The overlay must say `LM STATE X0.3.10`; wait until `F`, `C`, `H`, and `G` are
