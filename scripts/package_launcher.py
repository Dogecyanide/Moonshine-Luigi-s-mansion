...
Usage: package_launcher.py --boot-dol boot.dol --out-zip out.zip \
                           [--source di|sd|usb] [--mod-bins mod_jp.bin ...]
"""
import argparse
import subprocess
import sys
...
def render_meta(source, regions):
    import jinja2
    template = jinja2.Template(META_TEMPLATE.read_text())
    return template.render(version=git_version(), source=source, regions=regions)
...
def main(argv):
    ...
    with zipfile.ZipFile(args.out_zip, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr(f"{APP_NAME}/meta.xml", render_meta(args.source, regions))
